...
{return LOG(a)/LOG(b);};}
var math_prop=["LN10","PI","E","LOG10E","SQRT2","LOG2E","SQRT1_2","LN2","cos","pow","log","tan","sqrt","asin","abs","max","exp","atan2","random","acos","atan","min","sin"];for(var i=0,h=math_prop.length;i<h;i++)
...
{var v=e.val();if(raw)return $.fbuilder.parseValStr(v);v=$.trim(v);return($.isNumeric(v))?$.fbuilder.parseVal(v):$.fbuilder.parseValStr(v);}
...
{try
{var ffocusable=pageObj.find(":focusable"),_wScrollTop=$(window).scrollTop(),_viewportHeight=$(window).height(),_scrollTop=formObj.offset().top;if(ffocusable.length)
{ffocusable[0].focus();}
...
{var v=e.val();if(raw)return($.isNumeric(v))?v:$.fbuilder.parseValStr(v);v=$.trim(v);v=v.replace(new RegExp($.fbuilder['escape_symbol'](this.currencySymbol),'g'),'').replace(new RegExp($.fbuilder['escape_symbol'](this.currencyText),'g'),'');return $.fbuilder.parseVal(v,this.thousandSeparator,this.centSeparator);}
...
{var f=el.id.match(/_\d+$/),e;if(f)e=$.fbuilder['forms'][f[0]].getItem(el.name);if(e)v=e.val();return this.optional(el)||v>=p;};$.validator.methods.max=function(v,el,p)
{var f=el.id.match(/_\d+$/),e;if(f)e=$.fbuilder['forms'][f[0]].getItem(el.name);if(e)v=e.val();return this.optional(el)||v<=p;};}},val:function()
...
{me.invalidDates[counter]=new Date(range[j]);counter++;}}}}}},get_hours:function()
...
return'<select id="'+me.name+'_hours" name="'+me.name+'_hours">'+str+'</select>:';},get_minutes:function()
{var me=this,str='',i=0,m;while((m=me.minMinute+me.stepMinute*i)<=me.maxMinute)
{if(m<10)m='0'+''+m;str+='<option value="'+m+'">'+m+'</option>';i++;}
return'<select id="'+me.name+'_minutes" name="'+me.name+'_minutes">'+str+'</select>';},get_ampm:function()
...
$('#'+me.name).val(str).change();},show:function()
...
p=$.extend(p,{beforeShowDay:(function(w,i){return function(d){return[me.validateDate(d,w,i),""];};})(me.working_dates,me.invalidDates)});dp.datepicker(p);if(!me.predefinedClick)dp.datepicker("setDate",dd);if(!me.validateDate(dp.datepicker("getDate"),me.working_dates,me.invalidDates))
{dp.datepicker("setDate",'');}},setDefaultTime:function()
{var me=this;function setValue(f,v,m)
...
setValue(me.name+'_hours',(me.tformat==12)?((time['hour']>12)?time['hour']-12:((time['hour']==0)?12:time['hour'])):time['hour'],(me.tformat==12)?12:me.maxHour);setValue(me.name+'_minutes',time['minute'],me.maxMinute);$('#'+me.name+'_ampm'+' [value="'+((time['hour']<12)?'am':'pm')+'"]').prop('selected',true);}},setEvents:function()
...
{return this.optional(e)||((minDate==null||date>=minDate)&&(maxDate==null||date<=maxDate)&&item.validateDate($(e).datepicker('getDate'),item.working_dates,item.invalidDates)&&item.validateTime(e,item));}
return true;}
catch(er)
{return false;}};if(!(date_format in $.validator.methods))$.validator.addMethod(date_format,validator);me.setDefaultDate();me.setDefaultTime();me.setEvents();me.set_date_time();},val:function(raw)
{raw=raw||false;var me=this,e=$('[id="'+me.name+'"]:not(.ignore)'),dformat=me.dformat.replace(new RegExp('\\'+me.dseparator,'g'),'/');if(e.length)
{var v=e.val(),rt;if(raw)return $.fbuilder.parseValStr(v);if(dformat=='yyyy/mm/dd'||dformat=='yyyy/dd/mm')
...
{e.each(function(){var t=$.fbuilder[(m)?'parseVal':'parseValStr'](this.value);if(!$.isNumeric(t))t=t.replace(/^"/,'').replace(/"$/,'');if(m)v=(v)?v+t:t;else v.push(t);});}
...
$.fbuilder.htmlDecode(label)+''+((me.required)?'<span class="r">*</span>':'')+'</span></label></div>'+
dlg+'</div><div class="clearer"></div></div>';},after_show:function()
{$(document).on('click','.cff-open-dlg',function(){var dlg=$(this).closest('.fields').find('.cff-dialog'),w=dlg.data('width'),h=dlg.data('height');dlg.removeClass('hide');if('undefined'==typeof w)w=MIN($(this).closest('form').width(),$(window).width(),dlg.width());if('undefined'==typeof h)h=MIN($(this).closest('form').height(),$(window).height(),dlg.height());dlg.data('width',w);dlg.data('height',h);dlg.css({'width':w+'px','height':h+'px','margin-top':(-1*h/2)+'px','margin-left':(-1*w/2)+'px'});});$(document).on('click','.cff-close-dlg',function(){$(this).closest('.cff-dialog').addClass('hide');});},val:function()
{var e=$('[id="'+this.name+'"]:checked:not(.ignore)');if(e.length)
{var t=$.fbuilder.parseValStr(e[0].value);if(!$.isNumeric(t))t=t.replace(/^"/,'').replace(/"$/,'');}
return(v)?(($.isNumeric(v))?v:'"'+v+'"'):0;}});$.fbuilder.controls['fCalculated']=function(){};$.extend($.fbuilder.controls['fCalculated'].prototype,$.fbuilder.controls['ffields'].prototype,{title:"Untitled",ftype:"fCalculated",predefined:"",required:false,size:"medium",eq:"",suffix:"",prefix:"",decimalsymbol:".",groupingsymbol:"",dependencies:[{'rule':'','complex':false,'fields':['']}],readonly:true,hidefield:false,show:function()
...
return result;},val:function(raw)
{raw=raw||false;var e=$('[id="'+this.name+'"]:not(.ignore)');if(e.length)
{var v=e.val();if(raw)return $.fbuilder.parseValStr(v);v=$.trim(v);v=v.replace(new RegExp($.fbuilder['escape_symbol'](this.prefix),'g'),'').replace(new RegExp($.fbuilder['escape_symbol'](this.suffix),'g'),'');return $.fbuilder.parseVal(v,this.groupingsymbol,this.decimalsymbol);}
...
{var _match,field_regexp=new RegExp('(fieldname\\d+'+suffix+')(_[cr]b\\d+)?(\\|r)?([\\D\\b])','i'),__ME__=(__ME__)?$.fbuilder['forms'][suffix].getItem(__ME__).val():'';$.fbuilder['currentFormId']=$.fbuilder['forms'][suffix].formId;eq='('+eq+')';while(_match=field_regexp.exec(eq))
